...
<template>
    <AppLayout title="Home">
        ...
        <section class="font-lora max-w-7xl mt-4 mx-auto px-4 w-full bg-gray-100 dark:bg-dots-lighter dark:bg-gray-900">
            <div class="bg-[#e8a876] dark:bg-[#6b4a31] h-full w-full rounded-[1.5rem] flex flex-col gap-y-8 lg:p-24 p-8">
                <article class="flex flex-col items-center order-1 px-8">
                    <div class="flex flex-col gap-y-4 max-w-lg">
                        <h1 class="rtl:font-cairo md:text-6xl text-4xl dark:text-white">{{ $t("Sports Equipment Collection.") }}</h1>
                        <div class="flex justify-center">
                            <p class="flex flex-row gap-x-0.5 items-center text-lg text-black dark:text-white">In this season, find your desire
                            </p>
                        </div>
                        <div class="flex justify-center">
                            <button
                            class="font-cairo px-8 py-4 border font-semibold border-black rounded-full bg-black hover:bg-black/90 text-white transition-colors drop-shadow w-fit mt-4">
                            {{ $t("Start Your Queries") }}
                            </button>
                        </div>

                            <!-- <div>
                                <label for="language-select">Select Language:</label>
                                <select id="language-select" v-model="selectedLanguage" @change="updateLanguage(selectedLanguage)">
                                    <option v-for="(language, index) in availableLanguages" :key="index" :value="language">{{ language }}</option>
                                </select>
                            </div> -->

                            <div class="locale-changer">
                                <label class="font-cairo font-semibold" for="language-select">{{ $t("Select Language") }}: </label>
                                <select v-model="$i18n.locale" @change="updateLanguage($i18n.locale)">
